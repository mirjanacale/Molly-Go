...
function displayHotelDetails(hotelInfo) {
  const hotelsDiv = document.getElementById("hotels");
  // Unified image fallback chain for hotel details
  const mainImage =
    hotelInfo.hotelImages?.find((image) => image.defaultImage === true)?.url ||
    hotelInfo.hotelImages?.[0]?.url ||
    hotelInfo.main_photo ||
    hotelInfo.thumbnail ||
    hotelInfo.image ||
    hotelInfo.imageUrl ||
    hotelInfo.media?.[0]?.url ||
    hotelInfo.photos?.[0]?.url ||
    `https://source.unsplash.com/featured/?hotel,${encodeURIComponent(
      hotelInfo.city || "destination"
    )}`;

  console.log(
    "🖼️ Hotel details image chosen for:",
    hotelInfo.name || "Unknown",
    "=>",
    mainImage
  );
  ...
  const facilitiesList = facilitiesArray
    .slice(0, 10)
    .map(
      (facility) => (typeof facility === "object" ? facility.name : facility) // Check if facility is an object or just a string
    )
    .join(", ");
        ...
        <div class='card'>
          <div class='flex items-start'>
            <div class='card-image'>
              <img src='${mainImage}' alt='hotel' onerror="this.src='https://via.placeholder.com/600x400?text=No+Image'">
            </div>
...
function displayPaymentInfo(data) {
  ...
  if (voucherTotalAmount && voucherTotalAmount > 0) {
    content += `<p>Voucher Total Amount: ${Math.round(
      voucherTotalAmount
    )} USD</p>`;
  }
