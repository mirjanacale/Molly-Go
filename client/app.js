...
document.addEventListener("DOMContentLoaded", function () {
  console.log(
    "🔧 app.js: DOM Content Loaded - Initializing hotel search functionality"
  );
});
                  ...
                  minRate.rates[0].cancellationPolicies.refundableTag == "NRFN"
					...
					<p class='flex flex-col mb-0'>
    					<span class=${minRate.rates[0].retailRate.total[0].amount}></span>
   						<span class=${
                minRate.rates[0].retailRate.suggestedSellingPrice[0].amount
              }></span>
   						<button class='price-btn' onclick="proceedToBooking('${minRate.offerId}')">
       						 <s>${minRate.rates[0].retailRate.suggestedSellingPrice[0].amount} ${
      minRate.rates[0].retailRate.suggestedSellingPrice[0].currency
    }</s>
        					BOOK NOW ${minRate.rates[0].retailRate.total[0].amount} ${
      minRate.rates[0].retailRate.total[0].currency
    }
    					</button>
					</p>
				</div>
